...
![Three faults and what driftwatch says about each](docs/detections.png)
...
![The same fault, before and after suppression](docs/one-fault-one-alert.png)

That figure is generated by `python docs/plot.py`, which calls the same
`analyse()` the API does — so it cannot claim behaviour the code does not
have.
